...
%need to edit more
To achieve our goals---BLAH BLAH---, our system design choice is hiding the complexity of implementation details and open to the developers the capability of customization. Figure \ref{fig:architecture} describes the internal architecture of a typical application using our programming model. Our system comprises of two main components: (1) \emph{Network Management} (\emph{WNB}) manages a connection between mobile devices by, socket management, and connection establishment. For application programmers, the \texttt{WiFiBroadcaster} interface of \emph{WNB} provides simple interfaces to establish a connection; (2) \emph{Execution Handler} is in charge of selecting available peers and distributing the job to other peers. Specifically, the \texttt{JobDispatcher} which is the submodule of \texttt{JobHandler} dispatches a job and \texttt{JobExecutor} executes a job on each mobile device. Then, \texttt{JobHandler} merges the scattered incoming results into the final placeholder and relay information messages to the main application. In the following discussion, we present our system architecture.

\subsubsection{WiFi Network Manager}
To easily establish multiple connections between mobile devices over the WiFi network, we utilized the WiFi Direct technology, the new feature available from Android APIs 4.0. We developed the \texttt{WiFiDirectAdapter} sub module inside the \texttt{WNB} to wrap up the complexity of Wi-Fi Direct implementation. Holding an instance of \texttt{WifiP2pManager}, \texttt{WiFiDirectAdapter} maintains connectivity over peer-to-peer network, discover available peers and setup the connections. As aforementioned in previous section, to leverage cloud offloading when Wi-Fi access point (AP) is available, we implemented \texttt{OffloadEngine} module to handle connection with our cloud-based offloading server \cite{kwon+:mobilesoft15} to offload the jobs to available server instead of looking for the other devices. \texttt{OffloadEngine} will make transparent of the offload server and consider it as a typical device.

When the app starts up, \texttt{WNB} calls \texttt{discoverPeers()} to simultaneously search for available offloading server registered in its configuration file, as well as spread acknowledgments to inform its availability to the other peers. 

If any of offloading servers or devices receives such message, its \texttt{WNB} module will collect information from the new caller and update the list of devices, then send back their information in JSON object \ref{ss_dfp} to the caller. When all the devices are updated with their new device list, the network will be reformed (See figure \ref{fig:forming})
...
\texttt{Job} is united execution unit 
...
Our system has a simple programming model that enables the programmer can easily write remote functionality. First, as described in \ref{code:job_def}, we first provide a conventional interface that should be implemented by the programmer to specify what kinds of functionality should be executed remotely. Then, the implementation of \texttt{Job} is sent to a remote server or nearby devices along with data.

To support dynamic class loading in Dalvik VM, we provide the \texttt{DexCreator} tool that compiles the \texttt{Job} Java file into a Dex package. The Dex package and sliced resources are packed as a \texttt{JobData} object by \texttt{JobDispatcher} and then dispatched to the available peers. Once a peer receives a \texttt{JobData} object, it first checks the checksum field to confirm the consistency, and then deserializes the object into \texttt{Job} and resources for the execution.


\begin{figure}
\noindent \shadowbox{%
\begin{minipage}{249pt}
	\begin{lstlisting}
	public class Job {
...
In peer-to-peer network supported by our APIs, the server will distribute jobs to peers bases on investigating their availability by \texttt{DecisionMaker} (Figure \ref{fig:forming}). At the early stage before the transmission, the \texttt{DecisionMaker} of the calling device sends inquiring request for status (\texttt{IRS}) to some of selected peers in the network. If one receives this IRS request, its \texttt{DecisionMaker} will estimate its capability of response using the measurements of CPU, memory and battery usage at that time. 
...
\begin{figure*}
	...
	%\resizebox{0.45\textwidth}{!}{
		\includegraphics[width=0.45\textwidth]{data/img_small_perf_full.pdf}
