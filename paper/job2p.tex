...
\subsubsection{Estimate resource usages} \label{ss_ers}
...
Cloud offloading \cite{kwon+:mobilesoft15} plays a very important role in our system. When a peer found a nearby offloading server, it will attempt connecting to that server and offload the whole computation on it. 
...
As compared to estimating the energy consumption in cellular networks, in a Wi-Fi network the energy consumption can be easily estimated. The total energy consumption can be calculated as follows:
$$E_{0} = E + E_{wait}$$
Where $E_{wait}$ is the energy that the app requires for waiting. Then, we calculate the energy consumed by sending a job to other peers through the WiFi Direct technology as follows:
$$E_{p2p} = E(\frac{RL_{0}}{\sum_{i = 1}^{n}{RL_{i}}}) + E_{WiFi} + E_{wait}$$ 
\noindent $n$ is the number of devices. $RL_{i} (i = \overline{1,n})$ is the responsibility level of each mobile device. 

However, we do not consider $E_{wait}$ because it will depend on the appearance of applications. For example, if an application has no GUI, $E_{wait}$ will cause very little effect. From the two above equations, we can calculate the energy consumption differences between two job processing mechanisms:
$$E_{Diff} = E_{0} - E_{p2p}$$ 
or 
\begin{equation}
