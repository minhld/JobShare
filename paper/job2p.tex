...
To establish connections between nearby mobile devices over the WiFi network, we utilized the WiFi Direct technology, the new feature available from Android APIs 4.0. In particular, \texttt{WiFiDirectAdapter} of \texttt{WNB} hides the complexity of Wi-Fi Direct implementation. Holding an instance of \texttt{WifiP2pManager}, \texttt{WiFiDirectAdapter} discovers available peers, sets up a connection, and keeps track of the connection. As aforementioned in the previous section, to leverage computational offloading when a WiFi access point (AP) is available, the \texttt{OffloadEngine} module initiates a connection with our cloud-based offloading server \cite{kwon+:mobilesoft2015}, thus executing the job at a powerful remote server instead of looking for nearby devices. By providing transparent operations between the offload server and nearby mobile devices, our approach separates a job implementation from distributed execution models.

When an application starts, \texttt{WNB} calls \texttt{discoverPeers()} to search available offloading servers registered in its configuration file, as well as spread acknowledgments to inform its availability to the other peers. If any offloading server or nearby devices receive a discovery message, the \texttt{WNB} module updates the available device list with the information about the new caller, and then sends their information back to the caller in a JSON object \ref{ss_dfp}. When all the devices are updated with the available device list, the network is organized as described in Figure \ref{fig:forming}.
...
Our system has a simple programming model that enables the programmer can easily write remote functionality. First, as described in \ref{code:job_def}, we first provide an abstract class that should be extended by the programmer to specify which functionality should be executed remotely. Then, the \texttt{Job} class file is sent to a remote server or nearby mobile devices along with data.
