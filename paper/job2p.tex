...
According to the above estimations, we design the Algorithm \ref{alg:assign_job} for the \texttt{DecisionMaker} to assign appropriate job to each peer. It goes through 3 basic steps
...
\begin{itemize}
	...
	\item Use \texttt{DataParser.getSinglePart()} to split data into 
\end{itemize}

Algorithm \ref{alg:assign_job} describes how \texttt{DecisionMaker} assigns a job appropriately to each mobile device. The algorithm returns the amount of data in binary length with two offsets that indicate the location of the data.
