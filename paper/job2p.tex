...
Where $M$ is total size of data in bytes. This below algorithm \ref{alg:select_peer} will represent the procedure that \texttt{DecisionMaker} uses to select and reject peers, as well as assign the appropriate amount of job to them. 

\begin{algorithm}
\caption{CH election algorithm}
\label{alg:select_peer}
\begin{algorithmic}[1]
