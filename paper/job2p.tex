...
\subsubsection{WiFi Network Broadcaster}
To easily establish multiple connections between mobile devices over the WiFi network, we utilized the WiFi Direct technology, the new feature available from Android APIs 4.0. We developed the \texttt{WiFiDirectManager} sub module inside the \texttt{WNB} to wrap up the complexity of Wi-Fi Direct implementation. Holding an instance of \texttt{WifiP2pManager}, \texttt{WiFiDirectManager} maintains connectivity over peer-to-peer network, discover available peers and setup the connections. As aforementioned in previous section, to leverage cloud offloading when Wi-Fi access point (AP) is available, we implemented \texttt{WiFiOffloadManager} module to handle connection with our cloud-based offloading server \ref{kwon+:mobilesoft15} to offload the jobs to available server instead of looking for the other devices. \texttt{WiFiOffloadManger} will make transparent of the offload server and consider it as a typical device.
...
\begin{figure*}
	...
	%}
	\caption{Performance of the image processing app}
...
\begin{figure*}
	...
	%\resizebox{0.36\textwidth}{!}{
		\includegraphics[width=.45\textwidth]{data/gps_perf.pdf}
