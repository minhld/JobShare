...
In any IRS response, \texttt{availability} is determined by the remote peer which value can be either \texttt{on} or \texttt{off}. The \texttt{off} value informs the caller that it is not available due to temporary shortage of resource and therefore prevents the caller from sending jobs. Only the devices with \texttt{availability} with \texttt{on} value will accept jobs for execution. By default, if a device has the battery usage higher than 80\%, it will return \texttt{availability} \texttt{off}. 

The value is given based on whether the device has bypassed the threshold of battery usage which is by default set to 0.8 (or 80\% overall usage). User can override those thresholds by updating the \texttt{availability-thresholds} in the configuration file before app initialization . 
