...
Figure \ref{fig:architecture} shows the overall system architecture, which mainly consists of two parts: (1) \emph{Network Manager} discovers available mobile devices, checks the availability of the registered cloud-based remote servers, and then establishes a network connection for data transmissions; (2) \emph{Execution Handler} provides abstract functionality for job definition, execution, and distribution. For the optimal execution result, \emph{Execution Handler} keeps track of current execution environments such as network conditions (e.g., delay, bandwidth) and system status (e.g., resource usage including CPU, memory). Finally, to hide the complex implementation details for application programmers, we offer a straightforward programming model that can be easily imported or extended. In the following discussion, we present the details of the system architecture.

\subsubsection{Network Manager}
\emph{Network Manager} opens a peer to peer channel between nearby mobile devices over a WiFi network using WiFi Direct \cite{} or a direct communication channel to the cloud-based offloading server if it is more favorable than nearby peers. Specifically, the \emph{Network Manager} consists of three sub-modules including \texttt{Socket Handler}, \texttt{WiFi Direct Controller}, and \texttt{Offloading Engine}. First, \texttt{Socket Handler} manages a communication channel through an appropriate communication controller.

As aforementioned in the previous section, to leverage computational offloading, the \texttt{OffloadEngine} module initiates a connection with our cloud-based offloading server when a WiFi access point (AP) is available, thus executing the job at a powerful remote server instead of looking for nearby devices. By providing transparent operations between the cloud-based server and nearby mobile devices, our approach separates a business logic from distributed execution models.
...
Depend on each task, the number of peers will be decided correspondingly. Generally, system will try to contact as many peers as possible, but for some tasks like requesting GPS location data, only one peer which has the highest availability of resources will be invoked. In this case, the IRS criteria (Second code sniplet in Listing \ref{code:jsonResponse}) is added to the app configuration to support \texttt{DecisionMaker}. Based on these IRS responses and criteria, \texttt{DecisionMaker} will select the peers through 3 following steps
\begin{itemize} \denseitems
	\item \emph{If only one peer is requested}: Select one peer having max $RL$ and match the other criteria, then finish.
	\item \emph{If offloading server available}: Select one first available server, then finish.
	\item \emph{Otherwise}: Select a list of available peers having matched conditions.
\end{itemize}
...
Where $P_{AV}$ is a saving map of available peers and $WiFiDevices$ is the device list.\\

\subsubsection{Estimating Resource Usages} \label{ss_ers}
The CPU usage is calculated using the \texttt{/proc/stat} system file as follows:
\begin{equation} 
\label{eq:cpu_usage} \small
Usage_{CPU} = \frac{(\sum{T_{CPU2}} - T_{Idle2}) - (\sum{T_{CPU1}} - T_{Idle1})}{(\sum{T_{CPU2}} - \sum{T_{CPU1}})}
\end{equation}

Where $\sum{T_{CPU}}$ is total active time and $T_{Idle}$ is idle time. 

To calculate memory usage, we used \texttt{MemoryInfo} object from \texttt{ActivityManager} (available on Android SDK). After requesting system service, \texttt{MemoryInfo.availMem} returns the available amount of memory and \texttt{MemoryInfo.totalMem} returns the total amount of memory.

Regarding battery, by using hidden class \texttt{PowerProfile}, the capacity can be estimated from the \texttt{battery.capacity} property, while the usage can be retrieved by inquiring system service using \texttt{ACTION\_BATTERY\_CHANGED} intent.

Finally, we realized that the Responsibility Level (or $RL$) of a peer is proportional to resource capacity (ex. Number of cores, CPU speed or Total memory), and inversely proportional to resource usage. The more resources occupied, the more responding time the peer would take. Since we only consider the main system resources including CPU, memory and battery, we estimate $RL$ by the below equation
\begin{equation}
\label{eq:res_level}
\small
...
Where $N_{Cores}$ is the number of CPU cores. $CPU_{Speed}$ is speed of a single core in GHz. $Mem_{Spec}$ is the memory capacity in GB and $Batt_{Spec}$ is the battery capacity in mAh. For example, if $Usage_{CPU}$ is 0.3, $Usage_{Mem}$ is 0.5 (of half of 1GB memory has been consumed), and $Usage_{Batt}$ is 0.7 or 70\% used over a 2800uAh capacity battery, then $RL$ will be 
$$RL = \frac{5.2}{0.3} + \frac{1}{0.5} + \frac{2800}{0.7 \times 1000} = 23.33$$

The higher value of $RL$ means the higher availability of the peer. If the selected peer is a powerful cloud-based server with power wired, $Batt_{Spec}$ can be considered $\infty$ and $Usage_{Batt}$ is 0. According to Equation \ref{eq:res_level}, its $RL$ will be $\infty$. In our implementation, we assign the maximum value of Integer to $RL$ of offloading server. To reduce the calculation overhead on the caller side, $RL$ value is prematurely calculated on each peer and then sent back to caller in IRS responses.

\subsubsection{Job quantitation for peers}\label{ss_jqfp}

Before sending jobs to peers, \texttt{DecisionMaker} will quantitate the amount of data sending along with job to one peer based on $RL$ value it received from that peer. By doing that, our middleware can guarantee each peer will receive an appropriate job that is suitable to its capability.

Assume that $M_{i}$ is the appropriate amount of data sending to device $i$, the \texttt{DecisionMaker} will assign value by the following equation
\begin{equation} 
\label{eq:data_amount}
M_{i} = M\frac{RL_{i}}{\sum_{j = \overline{1,n}}{RL_{j}}}
\end{equation}

Where $M$ is total size of data in bytes. $n$ is the number of active peers, and $j$-peer has its Responsibility Level $RL_{j}$. According to this equation, in the network with an offloading server, we calculate the amount of data to send to the server $M_{offload}$
$$M_{offload} = M\frac{RL_{offload}}{\sum_{j = \overline{1, n}}{RL_{j}}} = M\frac{1}{1 + \frac{\sum_{j \neq offload}{RL_{j}}}{RL_{offload}}}$$

From the above Sub section \ref{ss_ers}, we got $RL_{offload}$ is $\infty$. Since $\sum_{j \neq offload}{RL_{j}}$ is a limited value due to peers' resource limits, therefore $M_{offload} \approx M$. Thus we can flush the whole task to the offloading server to gain the best performance. 

According to the above estimations, we design the Algorithm \ref{alg:assign_job} for the \texttt{DecisionMaker} to assign appropriate job to each peer. It goes through a few simple steps

\begin{itemize} \denseitems
	\item Calculate the total size of data ($M$) and total Responsibility capacity of all the peers ($RL_{Total}$).
	\item Estimate the length of data sending to each peer ($M_{i}$), indicated by \texttt{firstOffset} and \texttt{lastOffset}
	\item Use \texttt{DataParser.getSinglePart()} to split data into smaller part, merge it with \texttt{Job} class and then compact them into an instance of \texttt{JobData}.
	\item The \texttt{JobData} will be serialized and dispatched to a peer by \texttt{dispatchJob()} from \texttt{JobDispatcher}
\end{itemize}
...
Cloud offloading \cite{kwon+:mobilesoft2015} has received much attention as a popular energy/performance optimization mechanism. Thus, we still want to take advantage of a powerful cloud-based remote server for computation-intensive executions to achieve further energy/performance optimization. Specifically, if an offloading server is available, our middleware estimates costs (e.g., network cost) and benefits (e.g., CPU savings) to execute a job on the offloading server. Then, if executing the job on the offloading server is more favorable than executing peers in terms of energy- or performance-efficiency, the runtime system directly establish a communication channel between the offloading server and mobile device.
...
$$E_{p2p} = E(\frac{RL_{caller}}{\sum_{i \neq caller}{RL_{i}}}) + E_{WiFi} + E_{overload}$$ 
...
public interface DataParser {  
  ...
  public Object getSinglePart(..., numOfParts, sOffset, eOffset);    
...
\subsubsection{API Usage Scenario}
Next, we present how to implement an Android application using the provided APIs.
...
\textbf{\emph{Discovering Available Peers and Dispatching Jobs}}
...
public Object copyPartToPlaceholder(placeholder, 
	...
	\end{lstlisting}
\end{minipage}}
	\captionof{lstlisting}{Merging results in the image processing app.}
\end{figure}
...
\begin{figure*}
	...
	%\includegraphics[width=0.32\textwidth]{data/img_large_perf_full.pdf}
		\includegraphics[width=0.36\textwidth]{data/img_perf.pdf}
		\includegraphics[width=0.36\textwidth]{data/img_energy.pdf}
	%}
...
\begin{figure*}
	\centering
	%\resizebox{0.45\textwidth}{!}{
		\includegraphics[width=.40\textwidth]{data/net_perf_01.pdf}
		\includegraphics[width=.40\textwidth]{data/net_energy.pdf}
	%}
