...
The higher value of $RL$ means the higher availability of the peer. If the selected peer is a powerful cloud-based server with power wired, $Batt_{Spec}$ can be considered $\infty$ and $Usage_{Batt}$ is 0. According to Equation \ref{eq:res_level}, its $RL$ will be $\infty$. In our implementation, we assign the maximum value of Integer to $RL$ of offloading server. To reduce the calculation overhead on the caller side, $RL$ is prematurely calculated on each peer and then sent back to caller in IRS responses.

\subsubsection{Job quantitation for peers}\label{ss_jqfp}

To assign a job to multiple peers, the runtime system collects the available resource information from each mobile device. Specifically, 
