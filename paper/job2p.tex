...
\paragraph{Performance with a large scale image:}
...
\paragraph{Performance with a normal scale image:}
...
Lastly, we compare peer to peer- and offloading server-based executions in terms of performance and energy efficiency.
...
If a caller determines that executing a job at the offloading server is more beneficial than executing through nearby devices, it connects to the server and disconnects the communication channels established with the other peers. Then, the runtime system offloads the whole job to the server with data. One copy of the job will be maintained at the caller for handling faults.
...
\begin{itemize} \denseitems
	...
	\item The cloud offloading in WLAN environment (i.e., moderate latency/bandwidth), is sometimes more beneficial than the P2P-based offloading in terms of enery efficiency, but does not have performance benefits.
...
As a result, we can argue that different offloading mechanisms should be selected in accordance with different network conditions (i.e., latency/bandwidth characteristics) and different jobs (i.e., computation-intensive vs. sensor-based executions) in a dynamic, adaptive way to achieve further optimizations. 
%\cite{ELI}
