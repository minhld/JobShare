...
\begin{itemize} \denseitems
	...
	\item \textbf{A proof of concept infrastructure implementation:} Through the four case studies, our system demonstrates how mobile hardware resources can be effectively shared.
\end{itemize}
...
\noindent 
where $M$ is the total size of data in bytes, $n$ is the number of active peers, and $j$-peer has its \emph{Responsibility Level}, $RL_{j}$, respectively. If an offloading server is available, we calculate the amount of data sent to the server $M_{offload}$
...
\begin{itemize} \denseitems
	\item Calculate the total size of data ($M$) and the responsiveness level of all peers ($RL_{Total}$).
...
To measure the performance of the system equipped with our APIs, we experimented with four case studies using five different Android devices and one offloading server at an emulated testbed:
\begin{itemize} \denseitems
	\item \textbf{Image Processing:} In this case study, we initiated a P2P network to blur a large size image, which could not be loaded and processed on any single mobile device due to the limited memory space on mobile devices. Then, we distributed the blurring job to one to four mobile devices. 
...
\subsubsection{Image Processing}\label{eval_img_proc}
...
We experimented with the image processing case study with the same small-scale image. When increasing the latencies of both incoming and out-going packages of the server network from 0 to 500ms, we observed that the time and energy consumption also steadily increased (Figure \ref{fig:off_all}). By comparing with the previous results in Section \ref{eval_img_proc}, we have the following observations (Figure \ref{fig:off_compare_img}):
\begin{itemize} \denseitems
...
\centering
	...
	%}
	\caption{Compare experimental results of the Image processing between cloud offloading and P2P.}
