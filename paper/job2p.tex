...
Mobile devices have been steadily overtaking the personal computer as a primary means of accessing computing resource. Mobile hardware has been evolving at a lightning pace, with new devices introduced to the market at ever decreasing intervals and prices. 

%As a result, mobile software written yesterday may need to run on mobile devices to be designed tomorrow. In addition, the landscape of mobile devices is highly heterogeneous, with a panoply of devices having divergent hardware capacities. The same mobile application can be executed on mobile devices with different hardware configurations, varying in terms of their respective CPU, memory, and communication characteristics \cite{kwon+:infsof2013}. Another source of heterogeneity comes from mobile operating systems, whose implementation policies can affect the performance/energy profiles of the executed applications \cite{vahdat2000every}. Finally, mobile devices use a great variety of network types with divergent characteristics, including latency, bandwidth, congestion, packet loss, and interference. How can the programmer effectively improve the quality of service of a mobile application, executed on a variety of mobile devices across networks with divergent characteristics? Static, fixed adaptation strategies for mobile applications are unlikely to yield across-the-board benefits, and may even turn detrimental in some cases. 


%ensuring their quality of service remains a major challenge of mobile software development. The mobile applications' energy demands are outpacing the devices' battery capacities. In other words, rapid growth in application functionality requires ever greater energy budgets, thus subsuming any advances in battery capacities. Thus, energy efficiency (i.e., fitting an energy budget and maximizing the utility of applications under given battery constraints) has become an important software design consideration.

Smartphones are playing an important role in our daily life as a primary means of accessing computing resources. As a result of highly intensive completions between the global mobile phone manufactures like Samsung and Apple, the cellphones are more and more equipped themselves with high specifications (CPU, RAM and built-in storage) and top functionality so that they are powerful enough in compare with those in the past or even with a desktop. 
