...
\begin{figure}[H]
\centerline {
\includegraphics[width=0.5\textwidth, natwidth=939, natheight=564]{data/micro_01}
}
\caption{Energy consumption cumulation on caller within multiple clusters}
\label{fig:microb_24}
\end{figure}

According to the figure, it is revealed that in idle conditions without transmitting any data, the energy consuming variability on each device within the P2P network is linear and mostly identical at any time, regardless of number of devices to which it connects. This result is completely harmonized with \cite{wifi_energy} especially we transmit the same amount of data regularly over Wi-Fi connection.
...
\begin{figure}[H]
\centerline {
\includegraphics[width=0.5\textwidth, natwidth=552, natheight=273]{data/c_all_2}
}
\caption{Performance of clusters}
\label{fig:cluster_performance}
\end{figure}

To perform energy consumption on each cluster, we used Moosoon Power Monitor and measure the velocity of energy drained on a single mobile device.  

\begin{figure}[H]
\centerline {
\includegraphics[width=0.5\textwidth, natwidth=645, natheight=355]{data/energy_1}
}
\caption{Performance of clusters}
\label{fig:cluster_performance}
\end{figure}
