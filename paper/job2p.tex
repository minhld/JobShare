...
Since the mobile devices are the motive objects in the network, there is no guarantee that connections can be permanently maintained during the execution. To handle network malfunctions, as well as to ensure all jobs are completely executed by peers, we designed the \emph{HeartChecker} module to frequently check for heartbeat of all available peers. 

After dispatching a job to a peer, \emph{HeartChecker} will be enabled to periodically send heartbeat request to the peer in every $2$ seconds. After $10$ seconds listening without any response, the peer will be considered as disconnected for whatever reason. \emph{HeartChecker} will then remove it out of the available peer list and aware the dispatcher about the job incompleteness on that peer. Eventually the caller will handle that job and execute it locally. 
