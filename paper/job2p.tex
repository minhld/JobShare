...
\begin{itemize}
	...
	\item \textbf{Decision making module} determines the number of devices with according appropriate jobs for the remote execution bases on applied algorithm on level of availability.  
...
\subsection{Wi-Fi Peer-to-peer Broadcaster}
...
\subsection{Job Definition and Data Parser}
...
public interface DataParser {
  ...
  public byte[] parseObjectToBytes(Object objData);

  public Object parseBytesToObject(byte[] byteData);

  public Object getSinglePart(Object objData, 
                  int numOfParts, int firstOffset, 
                  int lastOffset);
    
  public Object createPlaceholder(String jsonMetadata);

  public Object copyPartToPlaceholder(
                  Object placeholder, byte[] partObj, 
                  int index);
...
\ForAll{$R_{IRS}$ in \{Incoming IRS responses\}}
\If {$R_{IRS}.availability = low$} 
   \State continue;
\Else
  \State $P_{Info} \leftarrow P2pDevice_{DeviceId} + R_{IRS}.RL$
...
Where $M$ is total size of data in bytes. This below algorithm \ref{alg:assign_job} will represent the procedure that \texttt{DecisionMaker} uses to select and reject peers, as well as assign the appropriate amount of job to them. 
...
\For {P in \{$P_{AV}$\}}
  \State $RL_{Total} \leftarrow RL_{Total} + P._{RL}$
\EndFor
\\
\State $firstOffset, lastOffset \leftarrow 0$
\State $jobData \leftarrow Null$
\State $job \leftarrow {readJobFromStorage()}$
...
\subsection{APIs for Android applications}
...
\subsubsection{Discovery Peer and Dispatching Jobs}
