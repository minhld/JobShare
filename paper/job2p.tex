...
In this section, we present the major technologies we used in this paper. The major technologies used in this work including computation offloading, remote execution, and middleware. We will describe them in the following sub sections.
...
In our initial prototype implementation, we provide three default parsers for bitmap images, text, and GPS, respectively. 
...
The calling peer quantitates the jobs from a task to distribute them to the other available devices in the network suitably. To achieve this, the $RL$ parameter will be used as the main coefficient for the split. 
...
Where $\sum{T_{CPU}}$ is total time of running CPU and $T_{Idle}$ is idle time correspondingly in hertz. In term of memory usage, the $Usage_{Mem}$ can be determined by using \texttt{MemoryInfo} from Android API to retrieve $Mem_{Avail}$ and $Mem_{Total}$, so the $Usage_{Batt}$.

Since in the mobile device, the lower resource usage state states the higher availability, and the higher specifications represent the better responsibility. Then the level of responsibility of device can be simply summarized by the expression below
...
Where $N_{Cores}$ is the number of CPU cores, $CPU_{Speed}$ is speed of single core in GHz unit, $Mem_{Spec}$ has GB unit and $Batt_{Spec}$ has uAh unit. At a certain time in this device, if $Usage_{CPU}$ is 0.3, $Usage_{Mem}$ is 0.5 (half of 1GB memory consumed) and $Usage_{Batt}$ is 0.7 or 70\% used over a 2800uAh capacity battery, the value of $RL$ will be
$$RL = \frac{5.2}{0.3} + \frac{1}{0.5} + \frac{2800}{0.7 \times 1000} = 23.33$$

If the peer is offloading server with power wired, $Batt_{Spec}$ is considered as $\infty$ and $Usage_{Batt}$ is 0. According to Equation \ref{eq:res_level}, $RL$ will also be $\infty$. To make it easy for calculation, we assign $RL$ value of offloading server as maximum value of Integer.

To reduce the latency and to avoid miscalculation at the calling peer, $RL$ is prematurely calculated by each available called peer and wrapped up in the IRS response sent back to the caller. 
...
Where $M$ is total size of data in bytes. It is obvious that in the network with an offloading server, $M_{offload}$ is almost equal to $M$, therefore we can flush the full task with heavy computation to the server. This below algorithm \ref{alg:assign_job} will represent the procedure through which the \texttt{DecisionMaker} judges peer capability to assign appropriate job. Where $M_{i}$ is the quantity of job in binary length, $firstOffset$ and $lastOffset$ indicate the location of the data trunk in the whole.
...
\section{Evaluation}
\label{sec:eval}
This section describes the micro benchmarks and case studies that evaluate the effectiveness of our approach. The experimental setup includes BLAH BLAH BLAH. Table \ref{table:devices} shows the device-specific values.

\subsection{Micro Benchmark}
We started the evaluation with micro benchmarking energy consumption of the system when mobile devices maintain connections in idle state, to figure out the cost of network maintenance overhead. Using Moosoon Power Monitor device \cite{moosoon} to replace battery and measure on one caller, we evaluated overhead bases on accumulation of energy consumption by time. The Figure (\ref{fig:microb_24}) shows the outcome of the caller measuring from 0 to 100 seconds when preserving connections with 1, 2 and 3 other peers. 
...
We applied blur effect, one of the energy consuming image processes. We run the test repeatedly on two image inputs, the first image has size $2500 \times 1405$ and the second size is $4326 \times 2856$. For the second image, with 4 bytes allocated for each pixel, device must allocate around $50MB$ of heap to open and hold another $50MB$ for the result, thus leading to app crash all the time. The problem will only be solved by other collaborative peers to solve overload in terms of memory and energy consumption.
